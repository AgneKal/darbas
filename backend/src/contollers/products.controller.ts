...
export class ProductsController {
    ...
    static async insert(req:any, res:any){
        const sql = "INSERT INTO products(name, price, weekday) VALUES (?, ?, ?)";
        await pool.query(sql, [req.body.name, req.body.price, req.body.weekday]);
        res.status(201).json({
            "success":true
        })
    }
    ...
    static async update(req:any, res:any){
        const sql = "UPDATE products SET name=?, price=?, weekday=? WHERE id=?";
        ...
        try{
            await pool.query(sql, [req.body.name, req.body.price, req.body.weekday, req.body.id]);
            res.json({
                "success":true
            });
        } catch (error){
